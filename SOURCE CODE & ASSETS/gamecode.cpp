...
ErrorType Game::StartOfGame()
{
   ...
   // **********************************************************************
	...
	// create a new spaceship at 0,0
	Spaceship* pShip = new Spaceship();
	pShip->Initialise(Vector2D(0, 0));
	m_objectManager.Add(pShip);

	// multiply the current level number from LevelManager by 5
	// to get the amount of asteroids needed in this level
	// 
	// so level 1 = 5 asteroids, level 2 = 10 etc
	for (int i = 0; i < (m_levelManager.GetLevel() * 5); i++) {
		// create an asteroid and place it randomly on screen
		...
		pAsteroid->Initialise(pos, vel);
		// add object to objectmanager
		m_objectManager.Add(pAsteroid);
	}
...
ErrorType Game::Update()
{
   ...
   // *********************************************************************
	...
	// update methods called for objectmanager (and therefore all objects)
	m_objectManager.UpdateAll(float(gt.mdFrameTime));
	m_objectManager.RenderAll();

	m_objectManager.CheckAllCollisions(); // check collisions between objects

	m_objectManager.DeleteInactiveObjects(); // delete objects that are inactive

	m_levelManager.Update(); // update the levelmanager to check for level changes
...
// for creating objects and adding to objectmanager outside of this class
